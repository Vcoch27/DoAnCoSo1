...
public class ImageAvatar extends JComponent {
    ...
    public void setIcon(Icon icon) {
        this.icon = icon;
        repaint();  // Thêm gọi repaint() để cập nhật lại giao diện
    }
    ...
    private Image toImage(Icon icon) {
        ...
        return null; // Hoặc xử lý khác nếu icon không phải là ImageIcon
    }
    public static void displayImageFromBytes(byte[] imageBytes, ImageAvatar ia) {
		try {
			ByteArrayInputStream bais = new ByteArrayInputStream(imageBytes);
			BufferedImage image = ImageIO.read(bais);
			ImageIcon newIcon = new ImageIcon(image.getScaledInstance(180, 180, Image.SCALE_SMOOTH));
			ia.setIcon(newIcon);
		} catch (IOException e) {
			e.printStackTrace();
		}
